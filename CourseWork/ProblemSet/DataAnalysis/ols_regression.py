...
def predictions():
    ...
    df = pandas.read_csv(r'C:\NanoDegree-Udacity\NanoDegree-Udacity\Introduction to Data Science\Dataset\turnstile_data_master_with_weather.txt')
    weather_turnstile = df.copy()
    weather_turnstile['weekday'] = pandas.DatetimeIndex(weather_turnstile['DATEn']).weekday
    result = smf.ols(formula="ENTRIESn_hourly ~ weekday+precipi+mindewpti+maxdewpti+maxpressurei+UNIT+Hour+maxtempi+mintempi+fog", data=weather_turnstile).fit()
